...
<html>
	...
	<body style="background-image: url('<c:url value="/resource/images/background6.png"/>');">
							...
							<table id="tabelaDespesas" class="table">
									...
									<c:forEach var="despesa" items="${despesas}">
										...
										<c:choose>
											<c:when test="${despesa.estadoDespesa != 'PAGO'}">
												<td>
													<a href="form-altera?id=${despesa.id}"> Editar </a>
												</td>
												<td>
													<a href="remover?id=${despesa.id}"> Excluir </a>
												</td>
												<td>
													<a href="efetuar-pagamento?id=${despesa.id}"> Efetuar Pagamento </a>
												</td>
											</c:when>
										</c:choose>
									</c:forEach>
								...
								<tfoot>
									<tr>
										...
										<td></td>
										<td align=center>
											<label><fmt:formatNumber type="currency" value="${totalDespesa}" /></label>
										</td>
										<td></td>
										<td></td>
										<td></td>
        ...
        <script type="text/javascript">
			var pager = new Pager('tabelaDespesas', 5);
			pager.init();
